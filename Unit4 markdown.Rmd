...
df_long%>% filter(zipcode %in% topzips) %>%select(Sex, `Group Age`, Count) %>%  ggpairs(cardinality_threshold = 33120, aes(color = Sex))+theme_stata()+scale_color_tableau()+scale_fill_tableau()

df_long %>% filter(zipcode %in% topzips)%>% ggplot(aes(x = `Group Age`, y = Count, color = Sex))+geom_line()+facet_wrap(~zipcode)+theme_stata()+scale_color_tableau()

df_long %>% filter(zipcode %in% topzips)%>% ggplot(aes(x = Count, fill = Sex))+geom_boxplot()+facet_wrap(~zipcode)+theme_stata()+scale_fill_tableau()
