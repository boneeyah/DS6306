...
brew_state <- brewcomb %>% mutate(region=tolower(region)) %>%
  group_by(region) %>% count(region) %>% arrange(desc(n))
brew_state %>% mutate(region=str_to_title(region)) %>% kable(col.names = c("State","Breweries")) %>% 
...
beer_brewery %>% ggplot(aes(x=Beer_ID,y=ABV))+geom_point()+theme_clean()
...
ipa_ale <- c()
ipa_ale <-
  beer_brewery %>% mutate(Ale_type = ifelse(
    str_detect(beer_brewery$Style, regex("ipa", ignore_case = TRUE)),
    "IPA",
    ifelse(str_detect(
      beer_brewery$Style, regex("ale", ignore_case = TRUE)
    ), "Ale", "other")
  ))
...
n <- nrow(ipa_ale)
set.seed(6)
nsamp <- sample(1:n,round(n*.7))
ipa_train <- ipa_ale[nsamp,]
ipa_test <- ipa_ale[-nsamp,]

summary(as.factor(ipa_train$Ale_type))
classification <- knn(scale(ipa_train[,6:7]),scale(ipa_test[,6:7]),cl = ipa_train$Ale_type,k=35)
cm <- confusionMatrix(classification,as.factor(ipa_test$Ale_type),positive = "IPA")
cm
#add changes later
#change again
```
...
census_api_key("c95a20360aee06a9c146971d90046c8fe6a73035")
income_med2019 <- get_acs("county",variables = "B25099_001", year = 2019)

income_med2019$region <- gsub(".*,","",income_med2019$NAME) %>% str_to_lower() %>% str_trim(side = "left")
income_med2019$subregion <-  gsub("County,.*|Parish,.*","",income_med2019$NAME) %>% str_trim(side = "right") %>% str_replace_all("[[:punct:]]","") %>% str_to_lower()

income_med2019 <- income_med2019 %>% select(region,subregion,estimate)


brew_county <- brewcomb %>% mutate(region=tolower(region),subregion=tolower(County)) %>% 
  group_by(region,subregion) %>% count(subregion)

us_counties <- us_counties %>% left_join(income_med2019,by=c("region","subregion"))
us_counties <- us_counties %>% left_join(brew_county, by=c("region","subregion"))
...
us_counties %>% ggplot()+
  geom_polygon(aes(x=long,y=lat,group=group,fill=estimate),color = "gray90",size=.1)+
  coord_map(projection = "albers",lat0=45,lat1=55)+
  scale_fill_viridis_c()+
  theme(axis.line = element_blank(),
        axis.text = element_blank(),
...
sapply(us_counties, function(x)sum(is.na(x)))
